...
.info-container { width: 100%; height: 100%; position: relative; /* 改为 relative */ overflow: hidden; display: flex; }
...
@media (max-width: 768px) {
    ...
    .character-image img { height: 70vh; /* 稍微缩小立绘，给底部留出更多空间 */ object-fit: contain; }
    .character-line, .instrument-content, .character-head, .logo { display: none; }
    .line-container { top: auto; bottom: 95px; /* 再次向上移动 */ transform: none; height: auto; z-index: 10; }
    .name-line { font-size: 1.8rem; text-indent: 1em; }
    .romaji-line { font-size: 0.9em; }
    .character-desc { top: auto; bottom: 170px; /* 再次向上移动 */ width: 90%; left: 5%; font-size: 0.8em; background-color: rgba(0,0,0,0.6); padding: 8px; border-radius: 8px; z-index: 10; }
    .select-part { left: 0; bottom: 15px; /* 再次向上移动，确保完整显示 */ width: 100%; background-color: transparent; }
