...
watch(activeItem, (newItem) => {
    ...
    if (playStatu.value === 1) player.value.play();
});
watch(musicProgress, (newProgress) => { if (musicProgress.value >= 100) switchMusic(activeItem.value.index); });
...
const switchStatu = () => {
    if (playStatu.value === 0) { player.value.play(); playStatu.value = 1; updateProgress(); } 
    else { player.value.pause(); playStatu.value = 0; }
};
const onVolumeProgressClicked = (e) => {
    const progressBar = e.currentTarget; const clickPosition = e.offsetX; const progressBarWidth = progressBar.clientWidth; const percentage = (clickPosition / progressBarWidth) * 100; volumeProgress.value = percentage; player.value.volume = percentage / 100;
}
const onProgressClicked = (e) => {
    const progressBar = e.currentTarget; const clickPosition = e.offsetX; const progressBarWidth = progressBar.clientWidth; const percentage = (clickPosition / progressBarWidth) * 100; musicProgress.value = percentage; player.value.currentTime = (percentage / 100) * player.value.duration; updateProgress();
}
const secToMMSS = (sec) => {
    sec = sec | 0; let m = sec / 60 | 0; let s = sec % 60 | 0; m = m.toString().padStart(2, '0'); s = s.toString().padStart(2, '0'); return m + ':' + s
}
const volumeHandle = (num)=>{
    if(player.value.volume+num/100 > 1){ player.value.volume = 1; volumeProgress.value=100; return; }
    else if(player.value.volume+num/100 < 0){ player.value.volume = 0; volumeProgress.value=0; return; }
    player.value.volume+=(num/100); volumeProgress.value+=num;
}
onMounted(() => {
    const yourElement = document.querySelector('.player-select');
    if (!yourElement) return;
    yourElement.addEventListener('wheel', (e) => { e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation(); yourElement.scrollTop += e.deltaY * 0.5; });
});
</script>
    ...
    <div class="bg">
        ...
        <div class="player-container">
            <div class="player-select">
                <ul>
                    <li v-for="(music, index) in musics" :key="index" :class="{ 'active': activeItem.name === music.name }" @click="activeItem = music; updateProgress();">
                ...
                <div class="now-playing">
                    ...
                    <div class="player-controls">
                        <div class="volume-control">
                            <span class="icon-defuse" @click="volumeHandle(-10)"><img src="/assets/images/icon_defuse.png" /></span>
                            <div class="volume-progress-box" :style="{ '--volume-progress': volumeProgress + '%' }" @click="onVolumeProgressClicked($event)"><div class="volume-progress-fill"></div></div>
                            <span class="icon-add" @click="volumeHandle(10)"><img src="/assets/images/icon_add.png" /></span>
                        </div>
                        <div class="control-panel">
                            <span><img src="/assets/images/icon_like.png" /></span>
                            <span><img src="/assets/images/icon_mode.png" /></span>
                            <span class="mv-icon"><img src="/assets/images/icon_mv.png" /><span class="tooltip"><iframe :src="'//player.bilibili.com/player.html?isOutside=true&aid=516452760&bvid='+activeItem.bvid+'&cid=858089909&p=1'" scrolling="no" border="0" frameborder="no" framespacing="0" allowfullscreen="true"></iframe></span></span>
                        </div>
                        <div class="music-progress-container">
                            <span class="current-time">{{ secToMMSS(player.currentTime) }}</span>
                            <div class="music-progress-box" :style="{ '--music-progress': musicProgress + '%' }" @click="onProgressClicked($event)"><div class="music-progress-fill"></div></div>
                            <span class="duration-time">{{ activeItem.duration }}</span>
                        </div>
                        <div class="btn-bar">
...
<style scoped>
/* 桌面端样式保持不变 */
.bg { position: relative; overflow: hidden; width: 100vw; height: 100vh; display: flex; align-items: center; justify-content: center; background: linear-gradient(90deg, #ff86be 0%, #ffd859 25%, #5ad0ff 50%, #ff5656 75%); background-size: 300% 300%; animation: gradient 15s ease infinite; animation-play-state: var(--animation-state, paused); }
...
@media (max-width: 768px) {
    ...
    .album-image {
     ...
     .album-image:hover { /* 禁用手机上的悬浮放大效果 */
        width: 100px;
        height: 100px;
    }
